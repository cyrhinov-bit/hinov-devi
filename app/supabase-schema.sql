...
-- Table des utilisateurs étendus (Profils liés à auth.users)
...
CREATE TABLE quotes (
    ...
    status TEXT NOT NULL CHECK (status IN ('Brouillon', 'Envoyé', 'Accepté', 'Refusé', 'Révision')),
    client_comment TEXT,
    date TEXT NOT NULL,
...
-- 3. Row Level Security (RLS) - Pour le moment, accès total pour la maquette
...
-- Autoriser tout (Lecture/Ecriture) pour tous les utilisateurs authentifiés
CREATE POLICY "Activer tout pour les utilisateurs authentifiés" ON services FOR ALL TO authenticated USING (true);
CREATE POLICY "Activer tout pour les utilisateurs authentifiés" ON profiles FOR ALL TO authenticated USING (true);
CREATE POLICY "Activer tout pour les utilisateurs authentifiés" ON clients FOR ALL TO authenticated USING (true);
CREATE POLICY "Activer tout pour les utilisateurs authentifiés" ON prestations FOR ALL TO authenticated USING (true);
CREATE POLICY "Activer tout pour les utilisateurs authentifiés" ON settings FOR ALL TO authenticated USING (true);
CREATE POLICY "Activer tout pour les utilisateurs authentifiés" ON quotes FOR ALL TO authenticated USING (true);
CREATE POLICY "Activer tout pour les utilisateurs authentifiés" ON quote_lines FOR ALL TO authenticated USING (true);

-- Autoriser la lecture publique des profils (pour l'écran de connexion)
